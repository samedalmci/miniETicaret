...
import { Injectable, EventEmitter, Input, Output, ViewChild, ViewEncapsulation, Component } from '@angular/core';
...
const WASMREMOTE = "https://cdn.jsdelivr.net/npm/ngx-scanner-qrcode@1.7.6/wasm/index.js";
...
class NgxScannerQrcodeService {
    ...
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "19.2.0", ngImport: i0, type: NgxScannerQrcodeService, deps: [], target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "19.2.0", ngImport: i0, type: NgxScannerQrcodeService, providedIn: 'root' });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "19.2.0", ngImport: i0, type: NgxScannerQrcodeService, decorators: [{
            type: Injectable,
            args: [{
                    providedIn: 'root'
                }]
 ...
 * @returns
 */
const LOAD_WASM = (loadWasmUrl = '', as = new AsyncSubject(), renderer) => {
    let retry = 0;
    const LOAD_WASM_RETRY = (isLoadWasmRemote = false) => {
        const DONE = (isFirst) => {
            ...
            try {
                const END = () => {
                    if (isFirst) {
                        zbarWasm.setModuleArgs({ locateFile: (filename, directory) => loadWasmUrl ? loadWasmUrl : directory + filename });
                    }
...
class NgxScannerQrcodeComponent {
    ...
    */
    ...
    overrideConfig() {
        ...
        if ('isMasked' in this.config)
            this.isMasked = this.config.isMasked;
        if ('unScan' in this.config)
            this.unScan = this.config.unScan;
        if ('loadWasmUrl' in this.config)
            this.loadWasmUrl = this.config.loadWasmUrl;
        if ('symbolType' in this.config)
            this.symbolType = this.config.symbolType;
...
export { LOAD_WASM, NgxScannerQrcodeComponent, NgxScannerQrcodeService, ScannerQRCodeConfigType, ScannerQRCodeOrientation, ScannerQRCodeResult, ScannerQRCodeSymbolType };
